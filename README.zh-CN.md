...
点击 [使用此模板](https://github.com/Mai0313/gitea_bot/generate) 后即可开始。
...
```bash
# 若 console script 名称为 "gitea_bot"
uvx gitea_bot
...
uv run poe main        # 执行 CLI（等同 uv run gitea_bot）
...
`pyproject.toml` 内提供 `gitea_bot` 与 `cli` 两个入口点。目前演示返回简单 `Response` 模型，可依需求替换。
