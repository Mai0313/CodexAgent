...
Click [Use this template](https://github.com/Mai0313/gitea_bot/generate) to start a new repository from this scaffold.
...
```bash
# If your console script is named "gitea_bot"
uvx gitea_bot
...
uv run poe main        # run CLI entry (same as uv run gitea_bot)
...
Console entry points are defined in `pyproject.toml` as `gitea_bot` and `cli`. The example returns a simple `Response` model; replace with your own CLI logic.
